...
class ClientDatabase:
    ...
    class KnownUsers:
        def __init__(self, user, is_online='OFFLINE'):
            self.id = None
            self.username = user
    ...
    class Contacts:
        def __init__(self, contact, is_online='OFFLINE'):
            self.id = None
            self.name = contact
            self.is_online = is_online
    ...
    def is_online(self):
        contacts_name = [contact[0] for contact in self.session.query(self.Contacts.name).all()]
        is_online = [contact[0] for contact in self.session.query(self.Contacts.is_online).all()]
        return contacts_name, is_online


    # Функция возвращающая онлайн или нет контакт

    # def is_online(self):
    #     return [contact[0] for contact in self.session.query(self.Contacts.is_online).all()]
