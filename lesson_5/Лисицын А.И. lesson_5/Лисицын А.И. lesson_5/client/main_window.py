from PyQt5.QtWidgets import QMainWindow, qApp, QMessageBox, QApplication, QListView, QTableView
...
class ClientMainWindow(QMainWindow):
    ...
    #     is_online_list = self.database.is_online()
    ...
    # ['alex', 'test2'], [False, False])

    def clients_list_update(self):
        contacts_list = self.database.is_online()
        print(contacts_list)
        self.contacts_model = QStandardItemModel()
        tv = QTableView()
        for i in contacts_list:
            item = QStandardItem(i[0])
            item_2 = QStandardItem(i[1])
            item.setEditable(False)
            item_2.setEditable(False)
            self.contacts_model.appendColumn([item, item_2])
